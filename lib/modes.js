...
function writeMockToDisk(response, path) {
  ...
  writeFile(path, serializedResponse);
}
...
function serializeResponse(proxy, res, data) {
  ...
  var path = utils.getMockPath(proxy, res.req.path, res.req.method);
...
module.exports = {
  ...
  mock: function(proxy, req, res) {
    ...
    var path = utils.getMockPath(proxy, req.url, req.method);
    var expandedPath = utils.expandMockPath(path);

    if (expandedPath.mockExists) {
      mockResponse(expandedPath.mock, proxy, req, res);
    } else {
      write404(req, res, expandedPath.record);
      serializeEmptyMock(proxy, req, expandedPath.record);
